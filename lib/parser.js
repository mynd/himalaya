...
function parse(state) {
  ...
  while (cursor < len) {
    ...
    const isClosingTag = (0, _compat.arrayIncludes)(options.closingTags, tagName);
    let shouldRewindToAutoClose = isClosingTag;

    if (shouldRewindToAutoClose) {
      const {
        closingTagAncestorBreakers: terminals
      } = options;
    ...
    const hasChildren = !(attrToken.close || (0, _compat.arrayIncludes)(options.voidTags, tagName));

    if (hasChildren) {
      const size = stack.push({
        tagName,
        children,
        position
      });
